<template>
 <div class="container mx-auto flex flex-col items-center bg-gray-100 p-4">
  ...
  <div class="fixed w-100 h-100 opacity-80 bg-purple-800 inset-0 z-50 flex items-center justify-center" v-if="preloader">
  ...
  <div class="container">
   ...
   <template v-if="tickers.length">
    ...
    <div>
      ...
      <button v-if="hasNextPage" @click="page = page + 1" class="my-4 mx-2 inline-flex items-center py-2 px-4 border border-transparent shadow-sm text-sm leading-4 font-medium rounded-full text-white bg-gray-600 hover:bg-gray-700 transition-colors duration-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 disabled">Вперед</button>
    </div>
    <div>
 ...
 export default {
  ...
  methods: {
   subscribeToUpdate(tickerName){
    setInterval(async () => {
     const f = await fetch(
             `https://min-api.cryptocompare.com/data/price?fsym=${tickerName}&tsyms=USD,JPY,EUR&api_key=e004f7b2af02f22f7f3ef33c8a7c11a2735db3a71667657f0e827af5114dc28b`
     )
   ...
   add() {
    ...
    this.tickers = [...this.tickers, currentTicker];//Обновляем ссылку на текущий массив
    this.filter = "";
